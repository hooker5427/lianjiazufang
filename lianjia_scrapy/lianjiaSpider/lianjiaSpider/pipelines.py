...
class csvPipline(object):
    def  __init__(self):
        self.csv_dir ='lianjiaSpider/data'

    def process_item(self, item, spider):
        print(item)
        myitem = dict(item)
        city = myitem.pop("city")
        area = myitem.pop("area")
        if  not   os.path.exists(   join( self.csv_dir , city) ):
            os.mkdir(  join( self.csv_dir , city) )

        with  open(  f"{self.csv_dir}/{city}/{area}.csv" ,mode='a'  , newline= "") as csvfile :
            writer = csv.DictWriter( csvfile , fieldnames= myitem.keys() )
...
class MysqlTwistedPipeline(object):
    ...
    def handle_error(self, failure, item, spider):
        print(failure)
        print ("insert into dbs eroor:" ,  item )
    
    def do_insert(self, cursor, item):
        insert_sql = """insert into lianjia.lianjia_spider(title, link, location, rent, apartment_layout, area, orientation, publish_time, unit_price, floor)
